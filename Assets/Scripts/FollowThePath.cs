...
// Questo script definisce il path che i 'Nemici' seguiranno.
...
public class FollowThePath : MonoBehaviour {
    ...
    //Si settano i parametri del path e poi si mandano i Nemici al punto di inizio
    public void SetPath() 
    {
        ...
        transform.position = NewPositionByPath(pathPositions, 0); //si inviano i nemici al punto di inizio
        if (!rotationByPath)
    ...
    private void Update()
    {
        if (movingIsActive)
        {
            currentPathPercent += speed / 100 * Time.deltaTime;     //si calcola la percentuale di percorso in accordo con la velocità
            ...
            {
                if (loop)                                   //quando loop è settata, si muovono i Nemici al punto di inizio, altrimenti distruggere e disattivare
                    currentPathPercent = 0;
                else
                {
                    Destroy(gameObject);           
